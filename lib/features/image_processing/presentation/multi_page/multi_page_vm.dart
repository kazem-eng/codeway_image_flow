...
/// Multi-page document builder ViewModel.
class MultiPageVM {
  MultiPageVM({
  ...
  final _state = const BaseState<MultiPageModel>.success(MultiPageModel()).obs;
  BaseState<MultiPageModel> get state => _state.value;

  MultiPageModel get model => _state.value.data ?? const MultiPageModel();

  final _uuid = const Uuid();

  void init(MultiPageProps props) {
    final page = DocumentPage(
      id: _uuid.v4(),
      originalBytes: props.originalBytes,
      processedBytes: props.processedBytes,
    );
    _state.value = BaseState.success(
      MultiPageModel(pages: [page], selectedIndex: 0),
    );
  }
  ...
  void reorderPages(int oldIndex, int newIndex) {
    ...
    _state.value = BaseState.success(
      model.copyWith(pages: pages, selectedIndex: selectedIndex),
    );
  }
  ...
  void removePage(int index) {
    ...
    _state.value = BaseState.success(
      model.copyWith(pages: pages, selectedIndex: newIndex),
    );
  }
